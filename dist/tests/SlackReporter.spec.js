...
const SlackReporter_1 = __importDefault(require("../src/SlackReporter"));
...
test.describe('SlackReporter - preChecks()', () => {
    ...
    test('showInThread only supported for bots not webhooks', async ({ fakeSlackReporter, suite, fullConfig, }) => {
        delete process.env.SLACK_BOT_USER_OAUTH_TOKEN;
        const cloneFullConfig = JSON.parse(JSON.stringify(fullConfig));
        cloneFullConfig.reporter = [
            [
    ...
    test('okToProceed flag is set to false when the custom on failure layout provided is not a Function', async ({ fakeSlackReporter, suite, fullConfig, }) => {
        const cloneFullConfig = JSON.parse(JSON.stringify(fullConfig));
        cloneFullConfig.reporter = [
            [
                '/home/ry/_repo/playwright-slack-report/src/SlackReporter.ts',
...
//# sourceMappingURL=SlackReporter.spec.js.map