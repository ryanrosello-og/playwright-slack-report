...
test_1.test.describe('ResultsParser - Parse failures', async () => {
    /**
    ...
    (0, test_1.test)('It returns an empty object when 2 tests pass', async () => {
        const teamTestPatterns = [
            { channelName: 'team0-channel', testNamePattern: '@team0' },
        ];
        const results = await resultsParser.getParsedFailureResultsByPattern(teamTestPatterns);
        (0, test_1.expect)(results).toHaveLength(0);
    });
    (0, test_1.test)('It returns an empty object when 2 tests are flaky', async () => {
        const teamTestPatterns = [
    ...
    (0, test_1.test)('It returns 1 entry with 2 elements when 2 tests fail', async () => {
        const teamTestPatterns = [
            { channelName: 'team2-channel', testNamePattern: '@team2' },
        ];
        const results = await resultsParser.getParsedFailureResultsByPattern(teamTestPatterns);
        test_1.expect.soft(results).toHaveLength(1);
        test_1.expect.soft(results[0].summary.failed).toEqual(2);
        test_1.expect.soft(results[0].summary.failures).toHaveLength(2);
        test_1.expect.soft(results[0].summary.tests).toHaveLength(2);
        test_1.expect.soft(results[0].summary.passed).toEqual(0);
        test_1.expect.soft(results[0].summary.flaky).toEqual(0);
        test_1.expect.soft(results[0].summary.skipped).toEqual(0);
    });
    (0, test_1.test)('It returns 1 entry with 1 element when 1 test fails and 1 test passes', async () => {
        const teamTestPatterns = [
            { channelName: 'team3-channel', testNamePattern: '@team3' },
        ];
        const results = await resultsParser.getParsedFailureResultsByPattern(teamTestPatterns);
        test_1.expect.soft(results).toHaveLength(1);
        test_1.expect.soft(results[0].summary.failed).toEqual(1);
        test_1.expect.soft(results[0].summary.failures).toHaveLength(1);
        test_1.expect.soft(results[0].summary.tests).toHaveLength(1);
        test_1.expect.soft(results[0].summary.passed).toEqual(0);
        test_1.expect.soft(results[0].summary.flaky).toEqual(0);
        test_1.expect.soft(results[0].summary.skipped).toEqual(0);
    });
    (0, test_1.test)('It returns an empty object when 1 test passes and 1 test is skipped', async () => {
        const teamTestPatterns = [
    ...
    (0, test_1.test)('It returns 1 entry for different test patterns with different channel names', async () => {
        const teamTestPatterns = [
            { channelName: 'team3-channel', testNamePattern: '@team3' },
            { channelName: 'team4-channel', testNamePattern: '@team4' },
        ];
    ...
    (0, test_1.test)('It returns 2 entries for different test patterns with same channel names', async () => {
        ...
        test_1.expect.soft(results[0].summary.failed).toEqual(2);
        test_1.expect.soft(results[1].summary.failed).toEqual(1);
        test_1.expect.soft(results[0].team).toEqual(teamTestPatterns[1].testNamePattern);
        test_1.expect.soft(results[1].team).toEqual(teamTestPatterns[2].testNamePattern);
    });
});
//# sourceMappingURL=parse_failure_results_by_pattern.spec.js.map